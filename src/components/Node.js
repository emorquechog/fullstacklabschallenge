...
const Node = ({node, expanded, toggleNodeExpanded}) => {
  ...
  const renderBlock = () => {
    if (node.blocks.length > 0 && !node.errorOnBlock) {
      return (
        node.blocks.map((data) => (
          <Block key={data.id} attribute={data.attributes} />
        ))
      )
    }
    ...
    >
      ...
      <ExpansionPanelDetails>
        <div className={classes.wrapperBlocks}>
          {node.blockIsLoading ? <CircularProgress /> : renderBlock()}
        </div>
      </ExpansionPanelDetails>
    </ExpansionPanel>
  );
